...
| `threshold` | number | — | Numeric threshold used by rate-based alert types (reserved for future use). |
| `windowMinutes` | number | — | Time window in minutes for rate-based alert types (reserved for future use). |
...
When a new issue triggers an email rule, Argus delivers an email notification to the configured address containing the issue title, level, culprit, and a direct link to the issue detail view in your dashboard.
