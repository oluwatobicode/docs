...
This endpoint uses **DSN key authentication**, not a session cookie. Pass your project's public key in the `X-Argus-Auth` request header. You can find your DSN public key in **Project Settings → Client Keys**.

```
X-Argus-Auth: YOUR_PUBLIC_KEY
```
...
curl -X POST https://your-domain.com/api/v1/ingest/PROJECT_ID/envelope \
  ...
  -H 'X-Argus-Auth: YOUR_PUBLIC_KEY' \
  -d '{
    "level": "error",
...
| Status | Meaning                                                                                  |
|--------|------------------------------------------------------------------------------------------|
| `200`  | Event accepted and queued for processing.                                                |
| `400`  | Invalid envelope — validation failed. The response body contains Zod field-level errors. |
| `401`  | Missing or invalid DSN public key in `X-Argus-Auth`.                                     |
