---
title: "Frequently Asked Questions about Argus"
sidebarTitle: "FAQ"
description: "Answers to common questions about Argus plans, SDK behavior, data retention, fingerprinting, and alert delivery."
---
...
<AccordionGroup>
  ...
  <Accordion title="How does error deduplication work?">
    When the Argus worker processes an incoming event, it computes a **SHA-256 fingerprint** from the top 5 stack frames of the exception. Each frame contributes its filename, function name, and line number to the hash.

    If the fingerprint matches an existing Issue, Argus increments that Issue's event count and records the new occurrence — you don't get a new Issue card in the dashboard for every individual error. If the fingerprint is new, Argus creates a new Issue and fires any matching alert rules.

    **Practical implications:**

    - The same bug triggering 10,000 times appears as one Issue with a count of 10,000, not 10,000 separate Issues.
    - If you fix a bug and deploy a new release, the new stack frames produce a different fingerprint and a new Issue — old and new occurrences are kept separate.
  ...
  <Accordion title="Can I send alerts to Slack or PagerDuty?">
    Yes — use a **webhook alert rule**. When a new issue matches the rule's conditions, Argus sends an HTTP POST with the issue payload to any URL you specify.
